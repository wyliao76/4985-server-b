...
Ensure(serializer, passes_this_test) {
    assert_that(1 == 1);
}

Ensure(serializer, fails_this_test) {
    assert_that(0 == 1);
}
...
TestSuite *tests()
{
    ...
    add_test_with_context(suite, serializer, passes_this_test);
    add_test_with_context(suite, serializer, fails_this_test);
